...
public class Inventory : MonoBehaviour
{
    ...
    {
        StopAllCoroutines();
        if (/*slotCount > -1*/inventoryTabList.Count > 0)
        {
            Color color = slots[0].selected_item.GetComponent<Image>().color;
            color.a = 0f;
            for (int i = 0; /*i <= slotCount*/i < inventoryTabList.Count; i++)
            {
                ...
                else if (itemActivated)
                {
                    if (inventoryTabList.Count > 0)
                    {
                        if (Input.GetKeyDown(KeyCode.DownArrow))
                        {
                            ...
                            //    selectedItem = -2;
                            //}
                            if (/*selectedItem < slotCount - 1*/selectedItem < inventoryTabList.Count - 2)
                                selectedItem += 2;
                            else
                                selectedItem %= 2;
                            theSound.Play(key_sound);
                        ...
                        else if (Input.GetKeyDown(KeyCode.RightArrow))
                        {
                            ...
                            //    selectedItem = -1;
                            //}
                            if (/*selectedItem < slotCount*/selectedItem < inventoryTabList.Count - 1)
                                selectedItem++;
                            else
                                selectedItem = 0;
                            theSound.Play(key_sound);
